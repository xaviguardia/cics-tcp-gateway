...
{
    while (nsessions > 0 && sessions[nsessions - 1].state == SS_FREE) {
        nsessions--;
    }
}
 ...
 * Returns 1 if a KICKS dispatch happened (caller may want to yield). */
static int session_poll(struct gwsess *s)
{
    ...
    if (s->state == SS_RDHDR) {
        ...
        if (s->comalen == 0) {
            /* header-only request, dispatch immediately */
            s->state = SS_RDDATA;
            s->got = 0;
        } else {
            s->state = SS_RDDATA;
    ...
    if (s->state == SS_RDDATA) {
        ...
        /* complete frame received --dispatch through KICKS */
